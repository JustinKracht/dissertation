...
Although the lack-of-fit between $\bm{\Sigma}$ and $\bm{\Omega}$ was of primary interest in this dissertation, some researchers might also be interested in the lack-of-fit between $\bm{\Sigma}$ and $\hat{\bm{\Omega}}$. XXXXXX

Figures reporting the distributions of RMSEA, CFI, TLI, and CRMR values representing the lack-of-fit between $\bm{\Sigma}$ and $\hat{\bm{\Omega}}$ (analogous to \@ref(fig:rmsea-distributions), \@ref(fig:cfi-distributions), \@ref(fig:tli-distributions), and \@ref(fig:crmr-distributions)) are provided in \@ref(sigma-hat-fit-indices). In addition to looking at the distributions of these $\hat{\theta}$ fit indices, I was also interested in how they related to the corresponding fit indices indicating lack-of-fit between $\bm{\Sigma}$ and $\bm{\Omega}$. In particular, I was interested in determining how $\textrm{RMSEA}_{\hat{\theta}}$ was related to RMSEA and how $\textrm{CFI}_{\hat{\theta}}$ was related to CFI.

To better understand the relationship between the observed $\textrm{RMSEA}_{\hat{\theta}}$ and RMSEA values from the simulation study, \@ref(fig:rmsea-theta-vs-theta-hat) shows scatter plots of $\textrm{RMSEA}_{\hat{\theta}}$ and RMSEA conditioned on each of the model-error methods and model fit conditions. The figure shows that the $\textrm{RMSEA}_{\hat{\theta}}$ values were almost always lower than the RMSEA values corresponding to the same $\bm{\Sigma}$ matrix, falling below the solid black line indicating where both values would be equal. The figure also shows that the differences between $\textrm{RMSEA}_{\hat{\theta}}$ and RMSEA were largest when model fit was Poor and smallest when model fit was Very Good. When model fit was Fair or Poor, the differences between the two fit index values could be quite stark. In the most extreme instance, a $\bm{\Sigma}$ matrix with Poor model fit had an $\textrm{RMSEA}_{\hat{\theta}}$ value of .006 and an RMSEA value of .308. The relationship between RMSEA and $\textrm{RMSEA}_{\hat{\theta}}$ also differed depending on the model-error method used. For instance, the CB method consistently produced $\bm{\Sigma}$ matrices with very similar RMSEA and $\textrm{RMSEA}_{\hat{\theta}}$ values, whereas the other model-error methods often produced solutions with very different RMSEA and $\textrm{RMSEA}_{\hat{\theta}}$ values. In theory, the CB method should have led to solutions with equal RMSEA and $\textrm{RMSEA}_{\hat{\theta}}$ values because 
