...
[TK: BAD TRANSITION, REVISE] Finally, the differences among the smoothing methods can be seen in \@ref(fig:c129-with-annotation), which shows heat-maps of the population correlation matrices with model error ($\mathbf{\Sigma}$) and the model-implied covariance matrix obtained from analyzing $\bm{\Sigma}$ ($\hat{\bm{\Sigma}}$) for each of the model-error methods. The correlation matrices corresponded to the condition with ten orthogonal major common factors, five items per factor, weak factor loadings of 0.3, and Poor model fit. This particular condition was chosen because it led to $\mathbf{\Sigma}$ matrices with RMSEA and CFI values that indicated different levels of qualitative model fit and highlights the differences between the model-error methods that incorporated a target CFI value ($\textrm{TKL}_{\textrm{CFI}}$ and $\textrm{TKL}_{\textrm{RMSEA/CFI}}$) and those that did not ($\textrm{TKL}_{\textrm{RMSEA}}$, CB, and WB).
...
They were chosen because they had large differences between CFI and RMSEA, but the W matrix didn't violate the constraints and the CB matrix wasn't indefinite.
