...
if (make_plots) {
  ...
  without_W_penalty <- par_grid %>%
    ...
    ggplot(aes(x = eps, y = nu, fill = value)) +
    geom_raster() +
  ...
  obj_func_plot <- without_W_penalty / with_W_penalty + plot_layout(guides = "collect")
  
  ggsave(filename = here("img/obj_func_landscape_ten_factor.png"),
         obj_func_plot,
         dpi = 320,
         height = 5.75,
         width = 8)
