...
Although the lack-of-fit between $\bm{\Sigma}$ and $\bm{\Omega}$ was of primary interest in this dissertation, the lack-of-fit between $\bm{\Sigma}$ and $\hat{\bm{\Omega}}$ might also be of interest to some researchers. Whereas the [XXXX CONTINUE HERE XXXX].

Figures reporting the distributions of RMSEA, CFI, TLI, and CRMR values representing the lack-of-fit between $\bm{\Sigma}$ and $\hat{\bm{\Omega}}$ (analogous to \@ref(fig:rmsea-distributions), \@ref(fig:cfi-distributions), \@ref(fig:tli-distributions), and \@ref(fig:crmr-distributions)) are provided in \@ref(sigma-hat-fit-indices). In addition to the distributions of the $\hat{\bm{\Omega}}$ fit indices, I was also interested in how they related to the corresponding fit indices indicating lack-of-fit between $\bm{\Sigma}$ and $\bm{\Omega}$. In particular, I was interested in determining how $\textrm{RMSEA}_{\hat{\bm{\Omega}}}$ was related to RMSEA and how $\textrm{CFI}_{\hat{\bm{\Omega}}}$ was related to CFI.

To better understand the relationship between the observed $\textrm{RMSEA}_{\hat{\bm{\Omega}}}$ and RMSEA values from the simulation study, \@ref(fig:rmsea-theta-vs-theta-hat) shows box-plots of $\textrm{RMSEA}_{\hat{\bm{\Omega}}}$ and RMSEA conditioned on each of the model-error methods, factor loading levels, and levels of model fit. The figure shows that the $\textrm{RMSEA}_{\hat{\bm{\Omega}}}$ values were almost always lower than the RMSEA values corresponding to the same $\bm{\Sigma}$ matrix, resulting in positive differences between RMSEA and $\textrm{RMSEA}_{\hat{\bm{\Omega}}}$. The vast majority of these differences were quite small, with a median difference of only 0.003. However, \@ref(fig:rmsea-theta-vs-theta-hat) shows that the differences between RMSEA and $\textrm{RMSEA}_{\hat{\bm{\Omega}}}$ were affected by the level of model fit such that the differences were largest when model fit was Poor and smallest when model fit was Very Good. In the most extreme instance, a $\bm{\Sigma}$ matrix with Poor model fit had an $\textrm{RMSEA}_{\hat{\bm{\Omega}}}$ value of .006 and an RMSEA value of .308, indicating very different qualitative interpretations of model fit. Thus, although the differences between RMSEA and $\textrm{RMSEA}_{\hat{\bm{\Omega}}}$ were often negligible, in some cases they were large enough to indicate completely different qualitative interpretations of model fit for RMSEA and $\textrm{RMSEA}_{\hat{\bm{\Omega}}}$.

```{r rmsea-theta-vs-theta-hat, out.width = '100%'}
#| fig.cap = "Box-plots of the differences between RMSEA and $\\textrm{RMSEA}_{\\hat{\\theta}}$  conditioned on model-error method, levels of model fit, and factor loading. Note that some outliers were omitted from the plot to aid visualization. TKL = Tucker, Koopman, and Linn; CB = Cudeck and Browne; WB = Wu and Browne."
...
In addition to being affected by the level of model fit, \@ref(fig:rmsea-theta-vs-theta-hat) also shows that the relationship between RMSEA and $\textrm{RMSEA}_{\hat{\bm{\Omega}}}$ was affected by the strength of the major common factor loadings. Moving from left to right in  \@ref(fig:rmsea-theta-vs-theta-hat), the differences between RMSEA and $\textrm{RMSEA}_{\hat{\bm{\Omega}}}$ values tended to decrease as factor loadings increased from 0.3 to 0.8. The figure also shows that there was an interaction between model-error method and factor loading such that the differences between RMSEA and $\textrm{RMSEA}_{\hat{\bm{\Omega}}}$ values decreased the most for the $\textrm{TKL}_{\textrm{RMSEA}}$ and CB methods as factor loadings increased, compared to the alternative model-error methods.

```{r rmsea-theta-vs-theta-hat-factors, out.width = '100%'}
#| fig.cap = "Box-plots of the differences between RMSEA and $\\textrm{RMSEA}_{\\hat{\\theta}}$  conditioned on model-error method and number of factors. Note that some outliers were omitted from the plot to aid visualization. TKL = Tucker, Koopman, and Linn; CB = Cudeck and Browne; WB = Wu and Browne."
