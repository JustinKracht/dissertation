...
One of the primary questions the simulation study was intended to answer was whether the five model-error methods produced solutions with different fit index values when used with the same error-free models and target RMSEA and CFI values. In this section, I report the distributions of the RMSEA, CFI, TLI and CRMR model-fit indices for solutions produced by each of the five model-error methods ($\textrm{TKL}_{\textrm{RMSEA}}$, $\textrm{TKL}_{\textrm{CFI}}$, $\textrm{TKL}_{\textrm{RMSEA/CFI}}$, CB, and WB).[^sigma-vs-sigma-hat] 

[^sigma-vs-sigma-hat]: All of the results reported in this section are for fit indices calculated for $\mathbf{\Sigma}$. Figures showing analogous results for fit indices calculated using $\hat{\mathbf{\Sigma}}$ --- the model-implied correlation matrix obtained by analyzing $\mathbf{\Sigma}$ --- are provided in \@ref(sigma-hat-fit-indices).
