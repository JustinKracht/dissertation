...
Box-plots of $\textrm{CFI}_{\Delta}$ conditioned on model-error method, level of model fit, and factor loading strength are shown in \@ref(fig:cfi-theta-vs-theta-hat). Similar to the previous results for $\textrm{RMSEA}_{\Delta}$, the figure shows that $\textrm{CFI}_{\Delta}$ values were largest when model fit was Poor and smallest when model fit was Very Good. Moreover, the figure also shows differences among the model fit indices. Of the model-error methods, the $\textrm{TKL}_{\textrm{RMSEA}}$ method generally led to the largest median $\textrm{CFI}_{\Delta}$ values
