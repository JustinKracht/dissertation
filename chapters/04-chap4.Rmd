...
The percent of cases where the constraints on $\mathbf{W}$ were violated for each level of number of factors, number of items per factor, factor loading strength, and model fit are shown in \@ref(fig:w-major-factors-plot) and reported in \@ref(tab:tab-major-minor-factors). Only results for the $\TKLrmsea$ were included because the $\TKLrmseacfi$ and $\TKLcfi$ error methods seldom led to solutions that violated the constraints on $\mathbf{W}$. In fact, only 24 out of 180,000 cases (0.01%) had violated $\mathbf{W}$ constraints for the $\TKLrmseacfi$ and $\TKLcfi$ methods combined. \@ref(fig:w-major-factors-plot) shows that the $\mathbf{W}$ constraints were violated most often when model fit was Fair or Poor, factor loadings were relatively low, and there were many total items (i.e., many factors and items per factor).
