...
if (make_plots) {
  ...
  cb_time_plot <- ggplot(times, aes(x = p, y = minutes)) +
    ...
    labs(x = "p",
         ...
         subtitle = "One-factor models with salient loadings of .6 and $\\rmseaOmega$ = 0.05") +
    theme_bw()
...
```{r comparison-of-converged-vs-non-converged, fig.cap = "$\\rmseaOmega$ and $\\cfiOmega$ values for cases where the model-error method was $\\textrm{TKL}_{\\textrm{RMSEA/CFI}}$ and model fit was Poor. Grey dots indicate cases where the L-BFGS-G algorithm converged; black dots indicate cases that did not converge and where a genetic algorithm was used instead. The dashed black lines indicate the target $\\rmseaOmega$ and $\\cfiOmega$ values."}
if (make_plots) {
