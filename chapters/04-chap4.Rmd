...
To better understand the relationship between the observed $\textrm{RMSEA}_{\hat{\bm{\Omega}}}$ and RMSEA values from the simulation study, \@ref(fig:rmsea-theta-vs-theta-hat) shows scatter plots of $\textrm{RMSEA}_{\hat{\bm{\Omega}}}$ and RMSEA conditioned on each of the model-error methods and model fit conditions. The figure shows that the $\textrm{RMSEA}_{\hat{\bm{\Omega}}}$ values were almost always lower than the RMSEA values corresponding to the same $\bm{\Sigma}$ matrix, falling below the solid black line indicating where both values would be equal. The figure also shows that the differences between $\textrm{RMSEA}_{\hat{\bm{\Omega}}}$ and RMSEA were largest when model fit was Poor and smallest when model fit was Very Good. When model fit was Fair or Poor, the differences between the two fit index values could be quite stark. In the most extreme instance, a $\bm{\Sigma}$ matrix with Poor model fit had an $\textrm{RMSEA}_{\hat{\bm{\Omega}}}$ value of .006 and an RMSEA value of .308. The relationship between RMSEA and $\textrm{RMSEA}_{\hat{\bm{\Omega}}}$ also differed depending on the model-error method used. For instance, the CB method consistently produced $\bm{\Sigma}$ matrices with very similar RMSEA and $\textrm{RMSEA}_{\hat{\bm{\Omega}}}$ values, whereas the other model-error methods often produced solutions with very different RMSEA and $\textrm{RMSEA}_{\hat{\bm{\Omega}}}$ values. 

In theory, the CB method should have led to solutions with equal RMSEA and $\textrm{RMSEA}_{\hat{\bm{\Omega}}}$ values because $\hat{\bm{\Omega}} = \bm{\Omega}$ when the CB method converges to a valid solution (see \@ref(population-model-fit)). However, in the simulation study the CB method produced RMSEA and $\textrm{RMSEA}_{\hat{\bm{\Omega}}}$ values that were often very close but not exactly equal. The differences between RMSEA and $\textrm{RMSEA}_{\hat{\bm{\Omega}}}$ were largest when model fit was Poor and smallest when model fit was Very Good. This can be seen in \@ref(fig:rmsea-diff), which shows histograms of the differences between RMSEA and $\textrm{RMSEA}_{\hat{\bm{\Omega}}}$ for $\bm{\Sigma}$ matrices generated using the CB method, conditioned on level of model fit.

```{r rmsea-diff, out.width = '100%'}
#| fig.cap = "Histograms showing the distribution of differences between RMSEA and $\\textrm{RMSEA}_{\\hat{\\bm{\\Omega}}}$ for $\\bm{\\Sigma}$ matrices generated using the Cudeck and Browne method, conditioned on level of model fit."

if (make_plots) {
  rmsea_diff_plot <- results_matrix %>% 
    filter(error_method == "CB") %>% 
    mutate(rmsea_diff = rmsea - rmsea_thetahat) %>% 
    group_by(model_fit_rec) %>% 
    ggplot(aes(x = rmsea_diff)) + 
    geom_histogram() + 
    facet_grid(~ model_fit_rec) +
    labs(x = latex2exp::TeX("$RMSEA - RMSEA_{\\hat{\\mathbf{\\Omega}}}$"),
         y = "Count") +
    scale_y_continuous(labels = scales::comma) +
    theme_bw()
  
  ggsave(filename = here("img/rmsea_diff_plot.png"),
         plot = rmsea_diff_plot,
         dpi = 320,
         height = 3,
         width = 6)
}

knitr::include_graphics(here("img/rmsea_diff_plot.png"))
```
...
+ Talk about how the model-error methods differ in terms of differences between theta and theta_hat fit indices.
-->

```{r rmsea-theta-vs-theta-hat, out.width = '100%'}
#| fig.cap = "Scatterplot of RMSEA and $\\textrm{RMSEA}_{\\hat{\\theta}}$ values conditioned on model-error method and model fit condition. The solid black line indicates where RMSEA and $\\textrm{RMSEA}_{\\hat{\\theta}}$ are equal. TKL = Tucker, Koopman, and Linn; CB = Cudeck and Browne; WB = Wu and Browne."
...
knitr::include_graphics(here("img/rmsea_vs_rmsea_thetahat.png"))
```

Some more text about the next figure...

```{r rmsea-theta-vs-theta-hat-nfac, out.width = '100%'}
#| fig.cap = "Scatterplot of RMSEA and $\\textrm{RMSEA}_{\\hat{\\theta}}$ values conditioned on model-error method, number of major common factors, and number of items per factor. Conditions with three factors were omitted to conserve space. The solid black line indicates where RMSEA and $\\textrm{RMSEA}_{\\hat{\\theta}}$ are equal. TKL = Tucker, Koopman, and Linn; CB = Cudeck and Browne; WB = Wu and Browne."
...
if (make_plots) {
  ...
  ggsave(filename = here("img/rmsea-theta-vs-theta-hat-nfac.png"),
         dpi = 320,
         height = 7)
}

knitr::include_graphics(here("img/rmsea-theta-vs-theta-hat-nfac.png"))
```

Some text about \@ref(fig:cfi-theta-vs-theta-hat).

```{r cfi-theta-vs-theta-hat, out.width = '100%'}
#| fig.cap = "Scatterplots of CFI and $\\textrm{CFI}_{\\hat{\\theta}}$ values conditioned on model-error method and model fit condition. The solid black line indicates where CFI and $\\textrm{CFI}_{\\hat{\\theta}}$ are equal. TKL = Tucker, Koopman, and Linn; CB = Cudeck and Browne; WB = Wu and Browne."
