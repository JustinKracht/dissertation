...
Finally, the number of major common factors and the number of items per factor had important effects on $\textrm{RMSEA}_{\Delta}$. These effects are shown in \@ref(rmsea-theta-vs-theta-hat-factors), which contains box-plots of $\textrm{RMSEA}_{\Delta}$ conditioned on number of factors and number of items per factor. The most important effects shown in \@ref(rmsea-theta-vs-theta-hat-factors) are the interaction effects between model-error method and the number of factors and between model-error method and the number of items per factor. The figure shows that the TKL-based methods were most affected by the number of factors and the number of items per factor, but that the direction of the effect depended on the particular method. The $\textrm{TKL}_{\textrm{RMSEA}}$ method [PICK UP HERE...] 
...
if (make_plots) {
  rmsea_diff_factors <- results_matrix %>%  
    ...
    scale_x_continuous(limits = c(0, .075)) +
    scale_y_discrete(labels = scales::label_parse()) +
    labs(x = latex2exp::TeX("$\\textrm{RMSEA}_{\\Delta}$"),
         y = "") +
