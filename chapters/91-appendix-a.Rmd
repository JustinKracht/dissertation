...
| $\hat{\mathbf{\Omega}}$ | The model-implied population covariance (correlation) matrix obtained from analyzing $\mathbf{\Sigma}$. |
...
| $\varepsilon_T$ or $\textrm{RMSEA}_{T}$ | A target RMSEA value. |
...
| $\mathbf{1}_{\bm{W}}$ | An indicator function that is equal to one when the constraints on the $\mathbf{W}$ matrix are violated and is equal to zero otherwise. |
