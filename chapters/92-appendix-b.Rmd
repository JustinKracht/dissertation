...
### Heatmap Representations of $\bm{\Sigma}$ and $\hat{\bm{\Sigma}}$ {#sigma-heat-maps}

#### Heatmap Representations of $\bm{\Sigma}$ and $\hat{\bm{\Sigma}}$ for the Condition with Ten Orthogonal Factors, Five Items per Factor, Weak Factor Loadings, and Poor Model Fit
...
{ \centering \includegraphics[width=.8\linewidth]{img/c129_R_plots_with_annotation.png} }

\caption{Correlation matrices with model error ($\mathbf{\Sigma}$) and the implied correlation matrix obtained by fitting the population major-factor model to $\mathbf{\Sigma}$ ($\hat{\mathbf{\Omega}}$) for each model-error method. Results are shown for a set of solution matrices from the condition with ten orthogonal factors, five items per factor, weak factor loadings of 0.3, and Poor model fit. TKL = Tucker, Koopman, and Linn; CB = Cudeck and Browne; WB = Wu and Browne.}
...
#### Heatmap Representations of $\bm{\Sigma}$ and $\hat{\bm{\Sigma}}$ for the Condition with Ten Orthogonal Factors, Ten Items per Factor, Weak Factor Loadings, and Poor Model Fit
...
\ffigbox[][][]{\includegraphics[width=.8\linewidth]{img/c156_R_plots_with_annotation.png}
\captionsetup{width=\textwidth, textfont={}, labelfont={it}, justification=raggedright, singlelinecheck=false, labelsep=period} 
\centering
...
\label{fig:c156-with-annotation}
