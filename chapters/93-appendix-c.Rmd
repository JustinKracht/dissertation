...
### Percent of Cases with Violated $\mathbf{W}$ Constraints

```{r tab-major-minor-factors-sup, results='asis'}
...
apa_table(
  x = percent_w_major_factors_table,
  col.names = c("Factors", "Items per Factor", "Loading",
                "Very Good", "Fair", "Poor"),
  col_spanner = list("Model Fit" = c(4, 6)),
  digits = c(0, 0, 1, 1, 1, 1),
  escape = FALSE,
  align = "rrrrrr",
  caption = "The percent of cases that violated the minor common factor loading constraints when the $\\TKLrmsea$ method was used.",
  font_size = "small"
)
```
...
if (fit_mods) {
  ...
  mmf_coef_tab <- texreg::texreg(
    ...
    caption.above = TRUE,
    fontsize = "small",
    caption = 'Coefficient estimates and standard errors (in parentheses) for the logistic regression model predicting the probability of producing a solution with violated $\\mathbf{W}$ constraints (i.e., at least one minor factor with more than two absolute factor loadings greater than 0.3).',
...
\@ref(tab:d1-coef-tab) gives a summary of the analysis of variance for the absolute difference between the observed and target $\rmseaOmega$ values ($|\textrm{RMSEA}_{\textrm{obs}} - \textrm{RMSEA}_{\textrm{T}}|$). Small values of $|\textrm{RMSEA}_{\textrm{obs}} - \textrm{RMSEA}_{\textrm{T}}|$ indicated that a model-error method recovered the target $\rmseaOmega$ value well, whereas large values indicated that a model-error method recovered the target $\rmseaOmega$ value poorly. All numeric predictors were scaled to have a mean of zero and a standard deviation of one to make the coefficient estimates more easily comparable.
...
apa_table(d1_apa$table,
          caption = "Analysis of Variance for the absolute difference between the observed and target $\\rmseaOmega$ values ($|\\textrm{RMSEA}_{\\textrm{obs}} - \\textrm{RMSEA}_{\\textrm{T}}|$).",
...
\@ref(tab:d2-coef-tab) gives a summary of the analysis of variance for the absolute difference between the observed and target $\cfiOmega$ values ($|\textrm{CFI}_{\textrm{obs}} - \textrm{CFI}_{\textrm{T}}|$). Small values of $|\textrm{CFI}_{\textrm{obs}} - \textrm{CFI}_{\textrm{T}}|$ indicated that a model-error method recovered the target $\cfiOmega$ value well, whereas large values indicated that a model-error method recovered the target $\cfiOmega$ value poorly. All numeric predictors were scaled to have a mean of zero and a standard deviation of one to make the coefficient estimates more easily comparable.
...
apa_table(d2_apa$table,
          caption = "Analysis of Variance for the absolute difference between the observed and target $\\cfiOmega$ values ($|\\textrm{CFI}_{\\textrm{obs}} - \\textrm{CFI}_{\\textrm{T}}|$).",
          note = "$\\hat{\\eta}^2_{G}$ denotes the estimate of the generalized eta-squared effect size.",
          placement = "hb",
          font_size = "small",
          align = "lrrrrrr")
...
\@ref(tab:d3-coef-tab) gives a summary of the analysis of variance for the sum of the absolute differences between the observed and target $\cfiOmega$ values and the observed and target $\rmseaOmega$ values, $D = |\textrm{RMSEA}_{\textrm{obs}} - \textrm{RMSEA}_{\textrm{T}}| + |\textrm{CFI}_{\textrm{obs}} - \textrm{CFI}_{\textrm{T}}|$. Small $D$ values indicated that a model-error method recovered both the target $\cfiOmega$ and target $\rmseaOmega$ values well, whereas large $D$ values indicated that a model-error method recovered one (or both) of the target $\rmseaOmega$ or $\cfiOmega$ values poorly. All numeric predictors were scaled to have a mean of zero and a standard deviation of one to make the coefficient estimates more easily comparable.
...
if (fit_mods) {
  ...
  fit_agreement_coef_tab <- texreg::texreg(
    ...
    caption.above = TRUE,
    fontsize = "small",
    caption = 'Coefficient estimates and standard errors (in parentheses) for the logistic regression model predicting the probability of producing a solution with qualitative fit index agreement.',
...
\@ref(tab:rmsea-delta-coef-tab) gives a summary of the analysis of variance for the difference between $\rmseaOmega$ and $\textrm{RMSEA}_{\bOmegaHat}$, $\textrm{RMSEA}_{\Delta} = \rmseaOmega - \textrm{RMSEA}_{\bOmegaHat}$. Small values of $\textrm{RMSEA}_{\Delta}$ indicated that a model-error method led to similar values of $\rmseaOmega$ and $\textrm{RMSEA}_{\bOmegaHat}$ whereas large values indicated that the two fit indices were quite different. All numeric predictors were scaled to have a mean of zero and a standard deviation of one to make the coefficient estimates more easily comparable.
...
\@ref(tab:cfi-delta-coef-tab) gives a summary of the analysis of variance for the difference between $\cfiOmega$ and $\textrm{CFI}_{\bOmegaHat}$, $\textrm{CFI}_{\Delta} = \cfiOmega - \textrm{CFI}_{\bOmegaHat}$. Small values of $\textrm{CFI}_{\Delta}$ indicated that a model-error method led to similar values of $\cfiOmega$ and $\textrm{CFI}_{\bOmegaHat}$ whereas large values indicated that the two fit indices were quite different. All numeric predictors were scaled to have a mean of zero and a standard deviation of one to make the coefficient estimates more easily comparable.
...
#| fig.cap = "Values of the TKL parameters ($\\epsilon$ and $\\nu_{\\textrm{e}}$) by model-error method, number of factors, number of items per factor, and factor loading strength when model fit was Poor."
...
### Observed $\rmseaOmega$ Values

```{r rmsea-distributions-full, fig.cap = "Distributions of the $\\rmseaOmega$ values for solutions produced by each of the model-error methods, conditioned on number of factors, model fit, and factor loading strength. The dashed lines indicate the target $\\rmseaOmega$ value for each condition.", out.width='70%'}
if (make_plots) {
  rmsea_distributions <- 
    results_matrix %>%
    filter(rmsea < 3) %>%
...
### Observed $\cfiOmega$ Values

```{r cfi-distributions-full, fig.cap = "Distributions of the $\\cfiOmega$ values for solutions produced by each of the model-error methods, conditioned on number of factors, model fit, and factor loading strength. The dashed lines indicate the target $\\cfiOmega$ value for each condition.", out.width='70%'}
if (make_plots) {
  cfi_distributions <- 
    results_matrix %>%
    filter(rmsea < 3) %>%
...
### Observed $\tliOmega$ Values

```{r tli-distributions-full, out.width="70%"}
#| fig.cap="Distributions of the $\tliOmega$ values for solutions produced by each of the model-error methods, conditioned on number of factors, model fit, and factor loading strength. The dashed lines indicate the threshold values of TLI that correspond to the targeted levels of model fit, according to Hu and Bentler (1999)."
...
### Observed $\crmrOmega$ Values

```{r crmr-distributions-full, fig.cap="Distributions of the $\\crmrOmega$ values for solutions produced by each of the model-error methods, conditioned on number of factors, model fit, and factor loading strength. Threshold values of 0.05 and 0.08 were proposed by Hu and Bentler (1999) as more and less conservative upper-bounds for acceptable SRMR/CRMR values; the dashed lines indicate these two threshold values.", out.width="65%"}
if (make_plots) {
  crmr_distributions <- 
    results_matrix %>%
    filter(rmsea < 3) %>%
...
#| fig.cap = "Distributions of the $\\textrm{RMSEA}_{\\hat{\\bm{\\Omega}}}$ values for solutions produced by each of the model-error methods, conditioned on number of factors, model fit, and factor loading strength. The dashed lines indicate the target $\\rmseaOmega$ value for each condition."
if (make_plots) {
...
#| fig.cap = "Distributions of the $\\textrm{CFI}_{\\hat{\\bm{\\Omega}}}$ values for solutions produced by each of the model-error methods, conditioned on number of factors, model fit, and factor loading strength. The dashed lines indicate the target $\\cfiOmega$ value for each condition."
if (make_plots) {
...
#| fig.cap = "The sum of the absolute differences between the observed and target $\\rmseaOmega$ and $\\cfiOmega$ values ($D$), conditioned on number of factors, model fit, and factor loading strength."
if (make_plots) {
  d_plot <- 
    results_matrix %>%
    filter(rmsea < 3) %>%
...
### Conditional Distributions of $\cfiOmega$ at Fixed $\rmseaOmega$ Values {#conditional-cfi-with-factor-cor}
...
### Conditional Distributions of $\rmseaOmega$ at Fixed $\cfiOmega$ Values {#conditional-rmsea-with-factor-cor}
...
#| fig.cap = "Observed $\\rmseaOmega$ and $\\cfiOmega$ values for the $\\textrm{TKL}_{\\textrm{RMSEA/CFI}}$ model-error method and the corresponding known-to-be-possible target $\\rmseaOmega$ and $\\cfiOmega$ values (indicated by the black lines), conditioned on number of factors, number of items per factor, factor loading strength, and factor correlation."
if (make_plots) {
